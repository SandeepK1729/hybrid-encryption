...
def otp_encrypt(text, key):
    ...
    text = text.upper()
    key = key.upper()
    # Ensure that the key is at least as long as the text
    if len(key) < len(text):
        raise ValueError("Key length should be at least the same as the text length.")
    # Perform character-wise XOR encryption
    encrypted_text = ""
...
def otp_decrypt(text, key):
    ...
    if len(key) < len(text):
        raise ValueError("Key length should be at least the same as the text length.")
    # Perform character-wise XOR decryption
    decrypted_text = ""
    for i in range(len(text)):
        char = text[i]
        key_char = key[i]
